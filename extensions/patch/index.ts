/**
 * Patch — Replace pi native edit/write with exact string replacement.
 *
...
const EditSchema = Type.Object({
  anchor: Type.Optional(Type.String({ description: "Optional unique string that appears BEFORE old_str in the file." })),
  old_str: Type.String({ description: "Exact text to find. Must be unique within the search range." }),
  new_str: Type.String({ description: "Replacement text. Use empty string to delete." }),
});

const PatchSchema = Type.Object({
  path: Type.String({ description: "Path to the file to edit (relative or absolute)." }),
  edits: Type.Optional(Type.Array(EditSchema, { description: "Targeted replacements applied sequentially." })),
  overwrite: Type.Optional(Type.Boolean({ description: "If true, replace the entire file atomically." })),
  new_str: Type.Optional(Type.String({ description: "Entire new file content when overwrite is true." })),
});
...
function preparePatchArguments(input: any): any {
  if (!input || typeof input !== "object") return input;
  const args = input as Record<string, any>;

  // Legacy multi-file: { patches: [...] } → extract first
  ...
  // Legacy: top-level old_str/new_str → wrap in edits[]
...
function appendPatchDiffChildren(parent: Box, body: string, theme: any): void {
  ...
  for (const line of rawLines) {
    if (line.startsWith("@@ lines ")) { flush(); parent.addChild(createSingleLineComponent(theme.fg("accent", line)) as any); continue; }
    if (line === "anchors:") { flush(); parent.addChild(createSingleLineComponent(theme.fg("accent", line)) as any); continue; }
    if (line.startsWith("  - ")) { flush(); parent.addChild(createSingleLineComponent(theme.fg("accent", line)) as any); continue; }
    buffer.push(line);
  }
  flush();
}
...
function buildPatchCallComponent(component: PatchCallComponent, args: any, theme: any, expanded = false) {
  ...
  if (args?.path) {
    ...
    if (args.overwrite) label += theme.fg("warning", " [overwrite]");
    else if (args.edits?.length > 0) label += theme.fg("dim", ` (${args.edits.length} edit${args.edits.length > 1 ? "s" : ""})`);
  }
